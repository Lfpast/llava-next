...
try:
    from transformers.trainer import xm, smp, met, amp
except ImportError:
    xm = smp = met = amp = None
...
def plot_graphs_based_on_log_history(log_history, output_dir, metrics=None):
    """
    Generate and save line plots for the training `log_history` produced by
    a :class:`~transformers.Trainer` subclass.

    Args:
        log_history (List[Dict]): entries from ``Trainer.state.log_history``.
        output_dir (str): directory where the plots will be written. The
            directory is created if it does not already exist.
        metrics (Optional[List[str]]): explicit list of metric keys to plot. If
            not provided, the function will automatically infer numeric metric
            names from the log history (excluding ``epoch``/``step``).
    """
    if not log_history:
        rank0_print("log_history is empty, nothing to plot.")
        return

    os.makedirs(output_dir, exist_ok=True)

    # figure out which metrics to draw
    if metrics is None:
        metrics = set()
